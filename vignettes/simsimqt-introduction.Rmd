...
The first parameter of a fitness function needs to be a vector of trait values, so that the selection functions can pass the trait value vector and get a fitness value back. Optionally, other parameters that control the shape of the fitness function can be included, and they can be passed ot the selection function, which will pass them on to the fitness function. Here, for example, the fitness function takes a parameter `s`.

```{r}
fitness_stabilising <- function(trait, s = 1) {
